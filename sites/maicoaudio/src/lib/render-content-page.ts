...
async function renderDynamicSitemap(html: string, route: string): Promise<string> {
	...
	const sitemapHtml = `<div class="simple-sitemap-wrap"><h3 class="post-type">Pages</h3>${renderSitemapList(
		uniquePages,
		"simple-sitemap-page",
	)}</div><div class="simple-sitemap-wrap"><h3 class="post-type">Posts</h3>${renderSitemapList(
		postItems,
		"simple-sitemap-post",
	)}</div>`;
...
export async function getRenderedContentPageHtml(route: string): Promise<string | undefined> {
	...
	const { entry } = await getEmDashEntry(config.collection, config.slug);
	if (!entry) {
