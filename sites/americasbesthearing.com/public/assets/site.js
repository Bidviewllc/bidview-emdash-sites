(function () {
  ...
  const buttonArrowIcon = (variant = 'right') => {
    if (variant === 'up') {
      return '<span class="abh-button-arrow abh-button-arrow-up" aria-hidden="true"><svg viewBox="0 0 14 14" xmlns="http://www.w3.org/2000/svg"><path d="M7 13V1"></path><path d="M2 6l5-5 5 5"></path></svg></span>';
    }
    return '<span class="abh-button-arrow" aria-hidden="true"><svg viewBox="0 0 18 14" xmlns="http://www.w3.org/2000/svg"><path d="M1 7h14"></path><path d="M11 2l5 5-5 5"></path></svg></span>';
  };
  ...
  function getSlidesToShow(root, options = {}) {
    const width = window.innerWidth;
    const forced = options.slidesToShow;
    if (forced) {
      const desktopForced = Number(forced.desktop || forced.laptop || forced.tablet || forced.mobile || 1);
      const laptopForced = Number(forced.laptop || desktopForced);
      const tabletForced = Number(forced.tablet || laptopForced || 1);
      const mobileForced = Number(forced.mobile || 1);
      if (width <= 767) return mobileForced || 1;
      if (width < 1024) return tabletForced || mobileForced || 1;
      if (width < 1366) return laptopForced || desktopForced || 1;
      return desktopForced || 1;
    }
  ...
  function setupSlider(root, options) {
    ...
    if (options.swiperStateClasses) {
      const widgetRoot = root.matches('.astro-widget-loop-carousel, .astro-widget')
        ? root
        : root.querySelector('.astro-widget-loop-carousel, .astro-widget');
    ...
    const shouldUseLoopClones = () => {
      if (!options.loopClones) return false;
      if (options.disableLoopClonesBelow && window.innerWidth < options.disableLoopClonesBelow) {
        return false;
      }
      return originalSlides.length > visible;
    };
    ...
    const rebuildLoop = () => {
      ...
      const shouldLoop = shouldUseLoopClones();
      if (!shouldLoop) {
        trackIndex = index;
        return;
      }
    ...
    const update = () => {
      visible = Math.max(1, Math.min(getSlidesToShow(root, options), originalSlides.length));
      rebuildLoop();
    ...
    const endDrag = (clientX) => {
      ...
      if (Math.abs(delta) > threshold) {
        ...
        update();
      } else if (dragLink && dragLink.href) {
        window.location.href = dragLink.href;
        dragLink = null;
        return;
      } else {
        setTranslate(dragStartTranslate, true);
      }
      dragLink = null;
      restart();
    ...
    viewport.addEventListener('pointerdown', (event) => {
      ...
      const isLink = event.target && event.target.closest && event.target.closest('a');
      if (isLink) {
        startDrag(event.clientX, isLink);
        try { viewport.setPointerCapture(event.pointerId); } catch (e) {}
        return;
      }
      event.preventDefault();
      startDrag(event.clientX, null);
      try { viewport.setPointerCapture(event.pointerId); } catch (e) {}
    });
  ...
  function setupTabs() {
    document.querySelectorAll('.astro-widget-n-tabs .e-n-tabs').forEach((tabs) => {
  ...
  function setupTableOfContents() {
    document.querySelectorAll('.astro-element-cdc3403, .elementor-element-cdc3403').forEach((tocWidget) => {
      const body = tocWidget.querySelector('.astro-toc__body, .elementor-toc__body');
      if (!body) return;

      const tocSection = tocWidget.closest('.astro-element-75a5bbc, .elementor-element-75a5bbc') || tocWidget;
      const tags = ['H2', 'H3', 'H4', 'H5', 'H6'];
      const candidates = Array.from(document.querySelectorAll('h2, h3, h4, h5, h6, .e-n-accordion-item-title-text'))
        .filter((node) => {
          const text = normalizeText(node.textContent);
          if (!text || text === 'table of contents') return false;
          if (tocSection.contains(node)) return false;
          if (node.closest('header, footer, .astro-location-header, .astro-location-footer')) return false;
          if (node.compareDocumentPosition(tocSection) & Node.DOCUMENT_POSITION_FOLLOWING) return false;
          return true;
        })
        .map((node) => {
          const isHeading = tags.includes(node.tagName);
          const level = isHeading ? Number(node.tagName.replace('H', '')) : 3;
          const target = isHeading ? node : (node.closest('summary') || node);
          const text = (node.textContent || '').replace(/\s+/g, ' ').trim();
          return { node, target, level, text };
        });

      if (!candidates.length) {
        body.innerHTML = '<div class="astro-toc__empty">No headings were found on this page.</div>';
        return;
      }
      ...
      candidates.forEach((item, index) => {
        ...
        let id = `astro-toc__heading-anchor-${index}`;
        while (usedIds.has(id)) {
          id = `astro-toc__heading-anchor-${index}-${usedIds.size}`;
        }
        usedIds.add(id);
        item.target.id = id;
      });
      ...
      rootList.className = 'astro-toc__list-wrapper elementor-toc__list-wrapper';
      const stack = [{ level: Math.min(...candidates.map((item) => item.level)), list: rootList }];
      ...
      candidates.forEach((item) => {
        ...
        if (item.level > stack[stack.length - 1].level) {
          ...
          if (parentItem) {
            ...
            childList.className = 'astro-toc__list-wrapper elementor-toc__list-wrapper';
        ...
        li.className = 'astro-toc__list-item elementor-toc__list-item';
        const wrapper = document.createElement('div');
        wrapper.className = 'astro-toc__list-item-text-wrapper elementor-toc__list-item-text-wrapper';
        const link = document.createElement('a');
        link.href = `#${item.target.id}`;
        link.className = `astro-toc__list-item-text elementor-toc__list-item-text${item.level === stack[0].level ? ' astro-toc__top-level elementor-toc__top-level' : ''}`;
  ...
  function setupFooterYear() {
    ...
    document.querySelectorAll('.astro-element-8b79fb7 .astro-heading-title').forEach((node) => {
      const text = node.textContent || '';
      if (!/©\s*\d{4}/.test(text)) return;
      node.textContent = text.replace(/(©\s*)\d{4}/, `$1${currentYear}`);
    });
  }
  ...
  function ensureButtonIcon(button) {
    ...
    const variant = button.closest('.astro-element-45c9425, .elementor-element-45c9425') ? 'up' : 'right';
    iconWrap.innerHTML = buttonArrowIcon(variant);
  }
  ...
  function setupContactForms() {
    document.querySelectorAll('form#gform_1').forEach((form) => {
      if (form.dataset.abhContactReady === 'true') return;
      form.dataset.abhContactReady = 'true';
      form.addEventListener('submit', async (event) => {
        event.preventDefault();
        const submit = form.querySelector('[type="submit"]');
        const existing = form.querySelector('.abh-form-message');
        if (existing) existing.remove();
        if (submit) submit.disabled = true;

        const notice = document.createElement('div');
        notice.className = 'abh-form-message';
        ...
        try {
          ...
          notice.textContent = error && error.message ? error.message : 'Something went wrong. Please try again.';
          notice.style.color = '#ec2028';
  ...
  ready(() => {
    ...
    setupSlider(document.querySelector('.astro-element-c69e7ab'), { autoplay: 6000, speed: 800, arrows: false, gap: 0 });
    setupSlider(document.querySelector('.astro-element-6e2ff94'), { autoplay: false, speed: 500, arrows: true, gap: 0, paginationAllSlides: true, loopClones: 'visibleMinusOne', disableLoopClonesBelow: 1025, swiperStateClasses: true, slidesToShow: { desktop: 4, laptop: 3, tablet: 2, mobile: 1 }, offset: { desktop: 0, laptop: 0, tablet: 0, mobile: 0 } });
    document.querySelectorAll('.astro-element-68060980').forEach((carousel) => {
      setupSlider(carousel, {
        autoplay: 5000,
        speed: 900,
        arrows: true,
        gap: 20,
        paginationAllSlides: true,
        loopClones: 4,
        swiperStateClasses: true,
        slidesToShow: { desktop: 5, laptop: 5, tablet: 4, mobile: 2 }
      });
    });
    document.querySelectorAll('.astro-element-614d5a0f').forEach((carousel) => {
      setupSlider(carousel, {
        autoplay: 5000,
        speed: 900,
        arrows: false,
    ...
    document.querySelectorAll('.astro-element-e37280d').forEach((carousel) => {
      setupSlider(carousel, {
        autoplay: 3500,
        speed: 2000,
        arrows: true,
        ...
        loopClones: 4,
        swiperStateClasses: true,
        slidesToShow: { desktop: 4, laptop: 4, tablet: 4, mobile: 2 }
      });
    });
    setupFooterYear();
    setupTabs();
