...
const LEAD_FROM_NAME = "WNC Audiology";
const SITE = "WNC Audiology";
...
export async function POST({ request }: { request: Request }) {
	...
	// Honeypot — input_3 must stay empty. Silently succeed if filled.
	if (get("input_3")) return ok();
	if (!(await turnstileOk(data, request))) return ok();

	const name = get("name", "input_4");
	const email = get("email", "input_7");
	const phone = get("phone", "input_6");
	const message = getRaw("message", "input_9");
	const extra = get("subject", "input_5");
