...
// Live listings fetched from the Trestle proxy. /api/listings is the canonical
// route (/listings is the human listings page, not the API).
const LOCAL_API = "";
...
function useFeaturedListings() {
  ...
  React.useEffect(() => {
    fetch(`${LOCAL_API}/api/listings`)
      .then(r => r.json())
      .then(d => {
        const raw = (d.listings || []).slice(0, 3);
        setListings(raw.map(l => ({
          ...
          tag: l.type || "RESIDENTIAL",
          img: l.photo || "/assets/listing-1056.jpg",
          live: true,
        })));
      })
      .catch(() => {})
...
function PropCard({ p, navigate }) {
  const handleClick = () => {
    if (p.live) {
      window.location.href = `/homes/${p.slug || p.id}/`;
    } else {
      navigate("listing", { id: p.id });
    }
  };
  return (
    <article className="prop-card" onClick={handleClick} style={{ cursor: "pointer" }}>
...
function Home({ navigate, emotionLayout = "centered", emotionPosition = "inline" }) {
  ...
  const { listings: featuredListings, loading: listingsLoading } = useFeaturedListings();
  ...
  return (
    <div className="page-fade">
      ...
      <section className="hero" data-screen-label="01 Hero">
        ...
        {/* Ambient dusk→night overlay (from Liz's approved prototype). The three
            layers animate opacity on a 36s cycle; hero__veil keeps text legible
            in every phase. */}
        <div className="hero__dusk" />
        <div className="hero__night" />
        ...
        <div className="hero__copy">
          ...
          <div className="hero__cta">
            <a className="btn btn--orange" href="/listings/">EXPLORE TAHOE HOMES</a>
            <a className="btn btn--ghost-light" href={PAGE_PATHS.contact}
      ...
      <section className="section section--white" data-screen-label="07 Featured Listings">
        <div className="container">
          ...
          <div className="props">
            {listingsLoading
              ? <div style={{ gridColumn: "1/-1", textAlign: "center", padding: "40px 0", color: "#75777E", fontSize: 14, letterSpacing: "0.08em" }}>Loading listings…</div>
              : featuredListings.map((p) => <PropCard key={p.id} p={p} navigate={navigate} />)
            }
          </div>
          <div style={{ textAlign: "center", marginTop: 56 }}>
            <a className="btn btn--dark" href="/listings/">
        ...
        <div className="container">
          ...
          <div className="micro">
            ...
            <div className="micro__content">
              ...
              <a className="reality__link micro__cta" href="/listings/">
                SEE LISTINGS HERE <Icon.ArrowRight />
              </a>
            </div>
          </div>
