...
const LEAD_FROM_NAME = "Audiology & Hearing Centers";
const SITE = "Audiology & Hearing Centers";
...
const CLINIC_MAP: Record<string, string> = {
	...
	"5970": "Palm Harbor, FL",
	"8513": "Tampa Palms, FL",
	"8514": "Tampa Westchase, FL",
	"8515": "Venice, FL",
	"8685": "Sarasota, FL",
	"11196": "Bonita Springs, FL",
	"8455": "Ocala, FL",
};
...
export async function POST({ request }: { request: Request }) {
	...
	// Honeypot — input_11 must stay empty. Silently succeed if filled.
	if (get("input_11")) return ok();
	...
	const clinicRaw = get("clinic", "input_10"); // location the visitor chose (multi-location practice)
	...
	const loc = clinic && !/required/i.test(clinic) && !/^clinic$/i.test(clinic) ? clinic : "";
